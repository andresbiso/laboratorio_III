...
int procesarEvento(int idColaMensajes, mensaje msg)
{
  switch(msg.intEvento)
  {
    ...
    case EVT_MENSAJE:
      ...
      printf("\n");
      enviarMensaje(idColaMensajes,MSG_DOS,MSG_UNO,EVT_FIN, "Chau Dos");
      return 0;
    case EVT_FIN:
      printf(msg.charMensaje);
      printf("\n");
...
int main(int argc, char *argv[])
{
  ...
  enviarMensaje(idColaMensajes,MSG_DOS,MSG_UNO,EVT_MENSAJE, "Hola Dos");
  levantaSemaforo(idSemaforo);

  while(1)
  {
    esperaSemaforo(idSemaforo);
    recibirMensaje(idColaMensajes, MSG_UNO, &msg);
